...
We want slicing to return some kind of a read-only "view" into the Biglist,
therefore it can not be the same `Biglist` class.
(For example, we don't want to support `append` on this view.)
For this reason, the `__getitem__` method does not accept a slice as the argument.

Instead, we can have a simple method `view`, which returns an instance of a different class---let's name it `BiglistView`.
`BiglistView` should support element access by index, slicing (which returns
a new `BiglistView` object), and iteration.
...
class BiglistView:
    ...
    def __getitem__(self, idx: Union[int, slice]):
        ...
        return self.__class__(
            self._list,
            view_slice_range(idx, self._range)
            )
...
There is a yet-to-be-defined function `view_slice_range`, which we'll come to in a bit.
...
which is the case in `Biglist.view()`.
However, once we slice the `BiglistView` object
(in `__getitem__` with an argument of type `slice`),
we need to work out the new (almost always smaller) window into the original Biglist.
...
In `v1`, the range is `0, 1,..., 9`.

In `v2`, the range is `0, 2, 4, 6, 8`.

In `v3`, the range is `6, 8`.

So, `BiglistView.__init__` expects a range in terms of the original, entire Biglist.
When we slice a `BiglistView`, we are slicing *that* range.
We need to turn this slice on the range into a simple range on the original Biglist.
With this simple range along with the Biglist, we create a new `BiglistView` object.
This happens in `BiglistView.__getitem__` (when the argument is a `slice` rather than an `int`), and is accomplished by the function `view_slice_range`---the name means
to "return a range for a slice on a view".

We do not often manipulate the Python classes `range` and `slice`.
This turns out to be some interesting little gymnastics.
...
(in a few dozen lines of code), until some days later,
...
```python
def view_slice_range(idx_on_view: slice, view_range: range):
    return view_range[idx_on_view]
```

Really this doesn't need to be a function!
...
Now we don't have to load this entire data into memory at once, and they don't need to be
contained in a single file. We have gained some scalability here.
Processing this dataset can take a long time, though.
A natural next step is to go parallel and process multiple segments of the list simultaneously.
...
First, it contains a reference to the "host" list. As we read elements of a `Biglist`,
...
A view can not *reference* the `Biglist` object. It has to read data *independent* of other views.

Second, it may come time when we want to use `multiprocessing` and send a `BiglistView` to another `Process`. When this happens, the `BiglistView` object will be pickled and transmitted across the process boundary. It is best that this object does not contain another object of a custom class, like `Biglist`, the reason being, as the design of `Biglist` evolves, who knows what kind of things it will loop in (and they will all needed to be pickled up and transmitted).

A solution is to not initiate `BiglistView` a object of `Biglist`, rather, just pass it the file *path* and let the `BiglistView` create a `Biglist` object itself.
Further, don't create the `Biglist` in `BiglistView.__init__`---which is run to initialize
the view object before it is sent to the other process.
...
Noticing the list is backed by a series of files, one logical way is to create a view for each file.
...
Further, we may as well add a method to iterate over a particular file.
We will use it in a `BiglistView` when that view is accessing exactly the elements of one file.
...
class Biglist:
    ...
    def iterfile(self, file_idx):
        ...
        if file_idx == self._read_buffer_file_idx:
            ...
            buffer = pickle_load(self.data_file(file_idx))
